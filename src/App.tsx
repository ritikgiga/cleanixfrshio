      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <Link to="/products" className={`nav-link ${currentPage === 'products' ? 'active' : ''}`}>Products</Link>
            <Link to="/about" className={`nav-link ${currentPage === 'about' ? 'active' : ''}`}>About Us</Link>
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-4 pt-2 pb-6 space-y-1">
              <Link to="/" onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-medium text-slate-700 hover:bg-slate-50 rounded-lg">Home</Link>
              <Link to="/products" onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-medium text-slate-700 hover:bg-slate-50 rounded-lg">Products</Link>
              <Link to="/about" onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-medium text-slate-700 hover:bg-slate-50 rounded-lg">About Us</Link>
              <Link to="/contact" onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-medium text-slate-700 hover:bg-slate-50 rounded-lg">Contact</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
const CheckoutPage = ({ cartTotal, onOrderPlaced }: { 
  cartTotal: number, 
  onOrderPlaced: () => void
}) => {
  const navigate = useNavigate();
  ...
  return (
    <section className="py-20 px-4 bg-slate-50 min-h-screen relative">
      <AnimatePresence>
        {showSuccess && (
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex-grow">
          <Routes>
            ...
            <Route path="/contact" element={<ContactPage contactSubmitted={contactSubmitted} setContactSubmitted={setContactSubmitted} />} />
